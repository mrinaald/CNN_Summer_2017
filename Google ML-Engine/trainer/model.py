...
DEFAULT_INCEPTION_CHECKPOINT = (
    'gs://dslab/mrinaal/Project/inception/inception_v3.ckpt')
...
def create_model():
  ...
  parser.add_argument('--label_count', type=int, required=True, help='Number of output labels')
  parser.add_argument('--dropout', type=float, default=0.5)
  parser.add_argument(
      '--inception_checkpoint_file',
      type=str,
      default=DEFAULT_INCEPTION_CHECKPOINT)
  parser.add_argument('--learning_rate', type=float, default=0.001, help='Learning rate used for optimization')
...
class Model(object):
  ...
  def __init__(self, label_count, dropout, inception_checkpoint_file, learning_rate):
  ...
  def build_graph(self, data_paths, batch_size, graph_mod):
    ...
    if is_training:
      tensors.train, tensors.global_step = training(loss_value, self.learning_rate)
    else:
  ...
  def export(self, last_checkpoint, output_dir):
    ...
    logging.info('Exporting prediction graph to %s', server_dir)
    logging.info('Exporting checkpoint files to %s', output_dir)
    ...
    with tf.Session(graph=tf.Graph()) as sess:
      ...
      builder = saved_model_builder.SavedModelBuilder(server_dir)
      ...
      save_path = saver.save(sess, os.path.join(output_dir, 'final.ckpt'))
      logging.info("Model saved in file: %s" % save_path)
      tf.train.write_graph(sess.graph_def, logdir=output_dir, name='final.pbtxt')
      ###########################################################################
